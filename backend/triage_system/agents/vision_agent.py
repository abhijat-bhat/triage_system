...
# Default: weights live in backend/assets/ (shipped with the repo).
# vision_agent.py path: <repo>/triage_system/backend/triage_system/agents/vision_agent.py
# parents[2] = backend/, so parents[2] / "assets" resolves to backend/assets/.
_DEFAULT_WEIGHTS_DIR = Path(__file__).resolve().parents[2] / "assets"
